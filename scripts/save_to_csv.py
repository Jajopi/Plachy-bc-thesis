...
def print_to_csv(results_or_resources=True, k=31, complements=True, subsampled=False, pangenomes=False, path_prefix=""):
    ...
    with open(out_file, "w") as file:
        if results_or_resources:
            file.write(CSV_SEPARATOR.join(("dataset", "method", "length", "runs", "objective", "relative", "compressed", "relativecompressed")) + '\n')
        else:
            ...
            for alg in (reversed(ALGORITHMS)):
                ...
                if results_or_resources:
                    file.writelines(map(lambda x: str(x).strip(), (
                        ...
                        f"{result[LABELS[5]]:3f}" if alg != ALG_OLD else "", CSV_SEPARATOR,
                        result[LABELS_COMPRESSIONS[2]], CSV_SEPARATOR,
                        f"{(result[LABELS_COMPRESSIONS[2]] / results[(ALG_OLD, inp, k, complements, None)][LABELS_COMPRESSIONS[2]]):3f}" if alg != ALG_OLD else ""
                    )))
                else:
